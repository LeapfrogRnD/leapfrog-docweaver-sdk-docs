...
# Explicitly import customJsonSchemaTypeError with an alias
from leapx.services.schema_generator.exceptions import JsonSchemaTypeError
...
def test_model_builder_constructor_with_undefined_arrays():
    ...
    schema = {
        "type": "object",
        "properties": {"data": {"type": "array"}},  # No items defined
    }

    # Should work without explicitly
    # passing allow_undefined_array_items to create_pydantic_model
...
def test_undefined_type():
    ...
    with pytest.raises(
        JsonSchemaTypeError,
        match="Schema must specify a type. Set allow_undefined_type=True",
    ):
        builder.create_pydantic_model(schema)
...
def test_json_schema_extra_with_user_example():
    ...
    # Original model as user defined - fixed deprecated Field usage
    class A(BaseModel):
        v: int = Field(
            ...,
            description="This is a field",
            json_schema_extra={"is_core_field": True},
        )
    ...
    assert (
        A.model_fields["v"].json_schema_extra
        == A_recon.model_fields["v"].json_schema_extra
    )

    # Test model functionality
    recon_v = 42
    instance = A_recon(v=recon_v)
    assert instance.v == recon_v
