...
def test_recursive_model_with_optional_reference():
    ...
    schema = {
        ...
        "type": "object",
        "properties": {"head": {"$ref": "#/definitions/LinkedNode"}},
        "required": ["head"],
    }
    ...
    instance = model(
        head={"data": "first", "next": {"data": "second", "next": {"data": "third"}}}
    )
...
def test_recursive_model_title_from_definition():
    ...
    instance = model(
        value=1, left={"value": 2, "left": {"value": 4}}, right={"value": 3}
    )
