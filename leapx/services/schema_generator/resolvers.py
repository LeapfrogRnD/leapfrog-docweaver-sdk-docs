...
from leapx.services.schema_generator.interfaces import IReferenceResolver, ITypeResolver
...
class TypeResolver(ITypeResolver):
    ...
    def resolve_type(
        ...
        if not isinstance(schema, dict):
            raise JsonSchemaTypeError(
                f"Invalid schema: expected dict, got {type(schema)}"
            )
        ...
        if isinstance(schema.get("type"), list):
            ...
            if "null" in types:
                ...
                return Optional[Union[tuple(resolved_types)]]  # noqa: UP007
            if len(types) == 1:
        ...
        if "enum" in schema:
            if not schema["enum"]:
                raise JsonSchemaTypeError("Enum must have at least one value")
        ...
        if not schema_type:
            ...
            if "properties" in schema:
                ...
                raise JsonSchemaTypeError(
                    "Schema must specify a type. Set allow_undefined_type=True to infer Any type for schemas without explicit types."
                )
