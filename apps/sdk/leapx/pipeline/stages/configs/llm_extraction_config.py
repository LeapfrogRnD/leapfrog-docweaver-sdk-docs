...
class LLMExtractionConfig(BlockConfig):
    """Configuration for the LLM extraction stage.

    Args:
        model: Target LLM model enum.
        provider: Backend extraction provider implementation.
        system_prompt: System prompt to steer extraction.
        json_schema: Pydantic model or JSON schema dict for extraction output.
    ...
    model: LLMModel
    provider: ExtractorProvider | None = ExtractorProvider.LITE_LLM
    ...
    def normalize_fields(cls, values: dict[str, Any]) -> dict[str, Any]:
        """Normalize convert a JSON schema dict to a model.

        Args:
            values: Raw initialization values for the pipeline config.

        Returns:
            Normalized  values with  schema converted.
        """
        schema = values.get("json_schema")

        if isinstance(schema, dict):
            values["json_schema"] = create_model(schema=schema)
        return values

    @classmethod
    def _infer_extraction_credential(
        cls, provider: ExtractorProvider
    ) -> LLMProviderType:
        """Infer the LLM Credential from extraction_provider.
        ...
        Raises:
            InvalidLLMModelError: If the credential type cannot be mapped to an
                LLM provider.
        """
        # Check if provider is already an ExtractorProvider enum
    ...
    def validate_credentials_after_init(self) -> LLMExtractionConfig:
    ...
    def get_extraction_config(self) -> dict[str, Any]:
        """Assemble the extraction request configuration for the ExtractorService.

        Returns:
            A dictionary suitable for constructing an ExtractionRequest.
        """
        model_value = (
            self.model.value if hasattr(self.model, "value") else str(self.model)
        )
        return {
            ...
            "user_prompt": {
                ...
                "context": "Below we have the OCR text of the document delimited by triple backticks:",
            },
            "config": {
